...
plot_all_niche_overlap_files_oil <- function(folder_path, output_folder = "FIGURES", plot_suffix = "_plot") {
  # List all files ending with _processed.csv in the given folder
  processed_files <- list.files(path = folder_path, pattern = "v2_processed\\.csv$", full.names = TRUE)
  ...
  for (file in processed_files) {
    ...
    plot_obj <- ggplot(data, aes(x = Predator_overlap, 
      ...
      geom_point(alpha = 0.5, show.legend = FALSE) +
      scale_size_continuous(range = c(1, 5))+
      scale_colour_gradient(low = "#682C23", high = "#661101") +
      geom_text_repel(size = 4, 
                      color = "black", 
                      family = "sans",
                      box.padding = unit(0.1, 'lines'),
...
plot_all_niche_overlap_files_oil("DATA/Niche_Overlap/", output_folder = "FIGURES/")
Niche_overlap_1990_oil <- `Refined PWS_v8_roms_Base1990_graph_Niche overlap_v2_processed`
plot_all_niche_overlap_files("DATA/Niche_Overlap/", output_folder = "FIGURES/")

plot_niche_overlap <- Niche_overlap_1990_oil/Niche_overlap_2010 / Niche_overlap_2016 

#grid::grid.draw(grid::textGrob(ylab,x=0.02, rot=90))
plot_niche_overlap <- plot_niche_overlap+
  plot_annotation(tag_levels= list(c("A ","B ","C ")))

ggsave("FIGURES/plot_niche_overlap_1990_2010_2016.png",plot_niche_overlap, width = 8, height = 10)
