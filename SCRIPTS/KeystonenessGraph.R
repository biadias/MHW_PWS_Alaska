...
for (yr in years) {
  ...
  p <- ggplot(keystone_index, aes_string(x = x_var, y = y_var, 
    ...
    scale_fill_gradient(low = "#682C23", high = "#682C23") +
...
plot_keystoneness <- p1996 / p2010  / pMHW  

#grid::grid.draw(grid::textGrob(ylab,x=0.02, rot=90))
plot_keystoneness <- plot_keystoneness+
  plot_annotation(tag_levels= list(c("A ","B ", "C ")))

#plot_keystoneness+ theme(plot.margin = unit(c(1.5, 1.5, 1.5, 1.5), "cm")) # Adjust margins as needed
